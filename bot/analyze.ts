// LLM analysis — standalone copy for bot
...
function getOpenAI(): OpenAI {
  if (!_openai) {
    ...
    if (!apiKey) throw new Error("TRUEFOUNDRY_API_KEY is not set");
    _openai = new OpenAI({
      apiKey,
...
- Action: ${signal.action.toUpperCase()} ${signal.shares.toLocaleString()} shares worth $${(signal.total_value / 1_000_000).toFixed(1)}M
...
${
  historicalPattern
    ? `HISTORICAL PATTERN for ${signal.ticker}:
